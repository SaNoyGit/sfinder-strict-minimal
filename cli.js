...
Usage:
  node cli.js -p <path> [-o <outputFile>] -a

Options:
  -p <path>  The filename of "path.csv" that is generated by solution-finder.
  -o <outputFile>  The path of the output file. [default: path_minimal_strict.md]
  -a output all combinations
`); //how to do npm stuff help
...
function outputall({filename, solutionArr, patternCount, successCount}) {
  textPiece = [];
  for (let i = 0; i < solutionArr.length; i++) {
    const solutions = solutionArr[i];
    Fumens = [];
    for (let j = 0; j < solutions.length; j++) {
      const solution = solutions[j];
      Fumens.push(solution.fumen);
    }
    textPiece.push(`### Combination ${i+1}
${Fumens.join("\n")}
`); //i don't care about the individual coverage, since all combinations are displayed, and I use it for checking 100% setups anways
  }
  fs.writeFileSync(filename, textPiece.join("\n"));
  console.log(`Write to ${filename}`);
}
